...
class Scrape < ApplicationRecord
    ...
    def self.get_default_owner
        owner = Owner.find_by(user_name:'username')
        if !owner.nil?
            Owner.create(first_name:'grant', last_name: 'smith', user_name:'username', password:'password')
        end
    end
    ...
    def self.get_flowers
        ...
        params = {name: flower_name, image: flower[1].css('img').attribute('src').value, description: flower[2].text, owner_id: Owner.find_by(user_name:'username').id, floristry_id: Floristry.find(Floristry.all[0].id).id}
